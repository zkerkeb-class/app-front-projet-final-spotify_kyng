...
const Home = () => {
  ...
  const fetchImageUrls = useCallback(async () => {
    try {
      const artistImagePromises = topArtists.map(async (artist) => {
        ...
        console.log('Image URL for artist:', artist.name, imageUrl);
        return imageUrl;
      });

      const albumImagePromises = topAlbums.map(async (album) => {
        const imageUrl = await getImageUrl(album.images?.[0]?.path);
        console.log('Image URL for album:', album.title, imageUrl);
        return imageUrl;
      });
  ...
  return (
    <Container>
      ...
      <h3 className="font-extrabold mt-10">Top 10 des derniers sons</h3>
      <HorizontalSlider>
        {tracks?.map((track) => (
          <PlaylistCard
            key={track._id}
      ...
      <h3 className=" font-extrabold mt-10">Top 10 des albums récents</h3>
      <HorizontalSlider>
        {topAlbums?.map((album) => {
